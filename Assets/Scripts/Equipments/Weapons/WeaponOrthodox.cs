...
public class WeaponOrthodox : Weapon {
    ...
    private float[] damageOfStraightScale = new float[MAX_LEVEL]    {  1.1f,    1.2f,    1.3f,    1.4f,    1.5f }; // 스트레이트 피해계수
    ...
    public override string Description => 
        level switch {
            >=5 => $"{interval[level]}초에 한 번 조준 방향을 향해 원 투 펀치를 날려 각각 {damageOfOneScale[level]*100}% / {damageOfTwoScale[level]*100}의 피해를 가합니다.",
            _   => $"{interval[level]}초에 한 번 조준 방향을 향해 원 투 스트레이트 펀치를 날려 각각 {damageOfOneScale[level]*100}% / {damageOfTwoScale[level]*100} / {damageOfStraightScale[level]*100}의 피해를 가합니다.",
        };
