...
public class WeaponChainSickle : Weapon {
    ...
    private float[] damageCoef = new float[MAX_LEVEL]              { 0.1f,    0.1f,    0.1f,    0.1f,    0.1f, }; // 피해계수
    private float[] staticDamage = new float[MAX_LEVEL]            {  30f,     45f,     60f,     75f,     90f, }; // 당기기 고정 피해량
    private float[] pullingDamageCoef = new float[MAX_LEVEL]       { 0.8f,    0.8f,    0.8f,    0.8f,    0.8f, }; // 당기기 피해계수
    private float[] pullingStaticDamage = new float[MAX_LEVEL]     {  15f,     20f,     25f,     30f,     35f, }; // 당기기 고정 피해량
    ...
    public override string Description =>
        (level+1) switch {
            5 => $"{interval[level]}초마다 조준 방향으로 자비 없는 사슬낫을 던져 적중하는 모든 적에게 <color=#f40>{staticDamage[level]}+{damageCoef[level]*100}%</color>의 피해를 가합니다.\n"
               + $"낫은 최대 사거리에 도달하면 그 자리에 꽂힙니다. 낫은 최대 3개까지 꽂혀 있을 수 있으며, 최대 개수를 초과하면 가장 먼저 던진 낫을 회수하며 <color=#f40>{pullingStaticDamage[level]}+{pullingDamageCoef[level]*100}%</color>의 피해를 입힙니다.\n"
