...
public class WeaponChainSickle : Weapon {
    ...
    public override string Description =>
        (NextLevelIndex+1) switch {
            5 => $"<nobr><color=#f40>{interval[NextLevelIndex]}</color>초마다 조준 방향으로 자비 없는 사슬낫을 던져 적중하는 모든 적에게 <color=#f40>{staticDamage[NextLevelIndex]}+{damageCoef[NextLevelIndex]*100}%</color>의 피해를 가합니다.\n"
            ...
            _ => $"<nobr><color=#f40>{interval[NextLevelIndex]}</color>초마다 조준 방향으로 자비 없는 사슬낫을 던져 적중하는 모든 적에게 <color=#f40>{staticDamage[NextLevelIndex]}+{damageCoef[NextLevelIndex]*100}%</color>의 피해를 가합니다.\n"
