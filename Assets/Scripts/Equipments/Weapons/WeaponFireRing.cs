...
public class WeaponFireRing : Weapon {
    ...
    private float[] staticDamage    = new float[MAX_LEVEL] {   25f,     35f,    35f,     55f,     65f }; // 고정 피해량
    ...
    public override string Description =>
        level switch {
            _ => $"적과 충돌하면 폭발하여 {staticDamage[level]}+{damageCoef[level]}%의 피해를 주는 화염구를 {(level<4 ? 3 : 5)}개 소환합니다.\n"
